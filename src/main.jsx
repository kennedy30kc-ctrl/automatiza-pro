...
import {
  ...
  Ojo, OjoDesactivado, Editar2, Basura2, Escudo, Gráfico de barras3, ChevronIzquierda,
...
const st = {
  ...
  lista asíncrona(prefijo, compartido = verdadero) {
    try { const r = await window.storage.list(prefix, shared); return r?.keys || []; } catch { return []; }
  }
};

// ═════════════════════════════════════════
// CONFIRMAR MODAL (reemplaza ventana.confirmar)
// ═════════════════════════════════════════
función ConfirmModal({ texto, onYes, onNo }) {
  devolver (
...
// MENSAJE FLASH
// ═════════════════════════════════════════
función FlashMsg({ msg }) {
  si (!msg.t) devolver null;
  devolver (
...
// BOTÓN DE RETROCESO
// ═════════════════════════════════════════
función BackBtn({ onClick, label = "Volver" }) {
  devolver (
    <button onClick={onClick} className="mb-5 flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-white" style={{ backgroundColor: "#334155" }}>
      <ChevronLeft size={15}/>{etiqueta}
    </button>
  );
}

// ═════════════════════════════════════════
// VISTA DE CÁMARA (componente independiente fuera de la aplicación)
// ═════════════════════════════════════════
función CamView({ color, videoRef, canvasRef, onStop }) {
  devolver (
...
// ═════════════════════════════════════════
// ═════════════════════════════════════════
// COMPONENTE DE VISTA DE TARJETA
// ═════════════════════════════════════════
función CardView({ emp, empresaNombre, empresaLogo, onClose, onPhotoUpload }) {
  const qrRef = useRef(null);
  const [qrReady, setQrReady] = useState(false);

  useEffect(() => {
    si (!qrRef.current) regresar;
    qrRef.current.innerHTML = "";
    const tryGen = () => {
      si (window.QRCode) {
        intentar {
          nueva ventana.QRCode(qrRef.current, {
            texto: JSON.stringify({ n: emp.nombre, c: emp.clave }),
            ancho: 120, alto: 120,
            colorOscuro: "#1e293b", colorClaro: "#ffffff"
          });
  ...
  const downloadCard = () => {
    constante W = 900; constante H = 520;
    const canvas = document.createElement("canvas");
    lienzo.ancho = A; lienzo.alto = H;
    ...
    const drawAll = (photoImg, logoImg) => {
      ...
      // ── Barra de encabezado superior (empresa) ──
      ...
      si (logoImg) {
        ...
        // círculo de marcador de posición con inicial
      ...
      ctx.textAlign = "derecha";
      ctx.fillText(new Date().getFullYear(), W - 24, 50);
      ctx.textAlign = "izquierda";

      // ── Acentuar la franja izquierda ──
      ...
      ctx.fillText("Desarrollado por Automatiza Pro · Nómina Activa", 24, H - 32);
    ...
    // Cargar foto y logotipo, luego dibujar
    const loadImg = (src) => new Promise((res) => {
      Si (!src) devuelve res(null);
      const img = new Image();
      img.onload = () => res(img);
      ...
      <div className="w-full max-w-lg mt-4">
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl mb-4 bg-white" style={{ border: "1px solid #e2e8f0" }}>
          ...
          <div className="flex gap-4 p-5" style={{ backgroundColor: "#f8fafc" }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="space-y-1" style={{ borderTop: "2px solid #e2e8f0", paddingTop: 8 }}>
                {[
                  ["Cédula", emp.cédula || "No registrado"],
                  ["Teléfono", emp.telefono || "No registrado"],
                  ["Correo", emp.correo || "No registrado"],
                  ["Dirección", emp.direccion || "No registrado"],
                ].map(([l, v]) => (
          ...
          <div className="px-5 py-2 flex justify-between items-center" style={{ backgroundColor: "#1e293b" }}>
            <p className="text-xs" style={{ color: "#3b82f6" }}>Desarrollado por Automatiza Pro · Nómina Activa</p>
        ...
        <botón
          ...
          deshabilitado={!qrReady}
          className="w-full py-4 rounded-xl font-bold text-white text-lg disabled:opacity-50"
          estilo={{ backgroundColor: "#1e293b" }}
        >
          {qr¿Listo? "⬇️ Descargar Carnet PNG" : "Generando QR..."}
        </button>
...
export default function App() {
  ...
  const [confirmar, establecerConfirmar] = useState(null); // { texto, onYes }

  // flujo de empleados
  const [empleadoId, setEmpleadoId] = useState(null); // Almacenar solo el ID, buscar datos nuevos
  ...
  // flujo de préstamos
  const [pasodepréstamo, establecerpasodepréstamo] = useState(1);
  const [montopréstamo, setMontoPréstamo] = useState("");
  const [préstamoPlazo, setPréstamoPlazo] = useState(1);
  const [freqpréstamo, setFreqpréstamo] = useState("mensual");
  const [préstamoTel, setPréstamoTel] = useState("");
  const [IdPréstamo, setIdPréstamo] = useState("");

  // jefe
  const [jefePass, setJefePass] = useState("");
  const [jefeSub, setJefeSub] = useState("menú");
  const [dashPeriodo, setDashPeriodo] = useState("hoy");
  const [permisoModal, setPermisoModal] = useState(nulo); // { empId, fecha, nombre }
  const [permisoNota, setPermisoNota] = useState("");
  const [alertaModal, setAlertaModal] = useState(nulo); // { empId, registroIdx, nombre }
  ...
  const [editEmpId, setEditEmpId] = useState(nulo);
  ...
  const [selEmpId2, setSelEmpId2] = useState(nulo);

  // ayudantes: obtener el empleado actual recién salido del estado de la empresa
  ...
  const init = async () => {
    ...
    si (id) {
      ...
      if (datos) { setEmpId(id); setEmpresa(datos); setScreen("principal"); devolver; }
    }
    establecerPantalla("configuración");
  };
  ...
  const resetFlow = useCallback(() => {
    setEmpleadoId(nulo); setEsNuevo(falso);
    setNombreInput(""); setClaveInput(""); setTelefonoInput("");
    setPréstamoPaso(1); setPréstamoMonto(""); setPréstamoPlazo(1); setLoanFreq("mensual"); setPréstamoTel("");
    detenerCam();
  }, []);
  ...
  const calcPréstamo = (monto, plazo, frecuencia) => {
    const m = parseFloat(monto) || 0;
    const int = m * 0.05 * plazo;
    const total = m + int;
    const n = frecuencia === "mensual"? plazo : frecuencia === "quincenal" ? plazo * 2 : Math.round(plazo * 4.3);
  ...
  const stopCam = () => {
    ...
    Si (rafRef.current) cancelAnimationFrame(rafRef.current);
    setCamActive(false);
  };
  ...
  const startCam = async () => {
    intentar {
      ...
      esperar a que videoRef.current.play();
      setCamActive(true);
      const loop = () => {
          ...
          si (window.jsQR) {
            ...
            si (código?.datos) {
              intentar {
                ...
                const emp = empresa?.empleados?.find(e => e.nombre === dn && e.clave === dc);
  ...
  const verificar = () => {
    ...
    if (!nombre || !clave) { flash("Completa nombre y clave", "err"); devolver; }
    // Buscar empleado existente (coincidir nombre + clave)
    let emp = empresa?.empleados?.find(e => e.nombre.toLowerCase() === nombre.toLowerCase() && e.clave === clave);
    si (!emp) {
      // Autorregistro de nuevos empleados
      emp = {
        id: Fecha.ahora(), nombre, clave,
        telefono: telefonoInput || "", cédula: "", correo: "", dirección: "",
    ...
    establecerEmpleoId(emp.id);
  };

  // ── ENTRADA (siempre lee datos actualizados) ──
  const entrada = () => {
    const ahora = new Date();
    const hoy = ahora.toLocaleDateString("es-ES");
    // Obtener datos de empleados RECIENTES de la empresa state
  ...
  const salida = () => {
    ...
    if (!ult) { flash("⚠️ No tienes entrada registrada hoy", "err"); setTimeout(ir a casa, 2000); devolver; }
    const hs = (ahora - nueva fecha(ult.hora)) / 3600000;
    const s = getSal(empleadoId);
    tarifa constante = s ? s.salarioMensual / s.diasLaborales / 8 : 0;
    const nuevaEmpresa = {
      ...empresa,
      empleados: empresa.empleados.map(e => e.id === empleadoId
  ...
  const confirmarLoan = () => {
    const emp = empresa?.empleados?.find(e => e.id === empleadoId);
    si (!emp) regresar;
    const préstamo = calcLoan(loanMonto, loanPlazo, loanFreq);
    ...
    ahorrar({
      ...
      prestamos: [...(empresa.prestamos || []), {
        ...
        montoSol: parseFloat(loanMonto), plazo: lendPlazo, frecuencia: lendFreq,
        intereses: parseFloat(préstamo.int), total: parseFloat(préstamo.total),
        nCuotas: préstamo.n, cuota: parseFloat(préstamo.cuota), cuotas,
        estado: "activo", fecha: new Date().toISOString(), teléfono: lendTel,
        // instantánea de los datos del empleado en el momento del préstamo
        empTelefono: emp.telefono, empCedula: emp.cedula, empCorreo: emp.correo, empDireccion: emp.direccion
      }]
    });
    establecerIDdelPréstamo(id);
    establecerPasoDePréstamo(4);
  };

  // ── JEFE ──
  const loginJefe = () => {
    if (jefePass === empresa?.claveJefe) { setScreen("jefe"); setJefePass(""); setShowPass(falso); }
  ...
  const saveEmp = () => {
    if (!fNombre.trim() || !fClave.trim()) { flash("Nombre y claves son obligatorios", "err"); devolver; }
    si (editEmpId) {
      // actualizar existente
      const nuevaEmpresa = {
        ...empresa,
        empleados: empresa.empleados.map(e => e.id === editEmpId
          ? { ...e, nombre: fNombre.trim(), clave: fClave.trim(), teléfono: fTel, cédula: fCed, correo: fCorreo, dirección: fDir, foto: fFoto || e.foto, perfilCompleto: !!(fCed && fCorreo) }
          : e),
        salarios: { ...empresa.salarios, [editEmpId]: { salarioMensual: parseFloat(fSal) || empresa.salarios[editarEmpId]?.salarioMensual || 250, diasLaborales: parseInt(fDias) || empresa.salarios[editarEmpId]?.diasLaborales || 20 } }
      };
      guardar(nuevaEmpresa);
      flash("✅ Empleado actualizado");
    } demás {
      // Comprobar si el nombre ya existe (evitar duplicados)
      const existe = empresa?.empleados?.find(e => e.nombre.toLowerCase() === fNombre.trim().toLowerCase());
      if (existe) { flash("⚠️ Ya existe un empleado con ese nombre. Edita el existente.", "err"); devolver; }
      const emp = {
        id: Fecha.ahora(), nombre: fNombre.trim(), clave: fClave.trim(),
        teléfono: fTel, cédula: fCed, correo: fCorreo, dirección: fDir,
        foto: fFoto,
  ...
  // ── NOMINA ACTIVA ──
  const loginNA = () => {
    if (naPass === CLAVE_MAESTRA) { loadEmpresas(); setScreen("nomina"); setNaPass(""); setShowPass(falso); }
    else flash("Clave incorrecta", "err");
  };
  ...
  const configurar = async () => {
    if (!sNombre.trim() || !sClave.trim()) { flash("Completa todos los campos", "err"); devolver; }
    if (sClave !== sConf) { flash("Las claves no coinciden", "err"); devolver; }
    if (sClave.length < 4) { flash("La clave debe tener un mínimo de 4 caracteres", "err"); devolver; }
    ...
    setEmpId(identificación); setEmpresa(nueva); setScreen("principal");
  };

  // ── AYUDAS DEL PANEL DE CONTROL ──
  ...
  const getDatesForPeriod = (periodo) => {
    ...
    si (periodo === "hoy") {
      ...
      Mientras (d <= hoy) { fechas.push(new Date(d)); d.setDate(d.getDate() + 1); }
    }
  ...
  const getEmpStatusForDate = (emp, date) => {
    ...
    if (!registro) return { estado: dateStr === hoy? "sin_registrar" : "falta", fecha: dateStr };
    const horaLlegada = nueva Fecha(registro.hora);
    ...
    si (!registro.salida) {
      const horasActivas = (nueva Fecha() - horaLlegada) / 3600000;
      return { estado: horasActivas >= 8 ? "horas_extra" : "en_turno", tarde, minutosTarde, horaLlegada: registro.horaReg, horasActivas: horasActivas.toFixed(1), registroRef: registro, fecha: dateStr };
    }
    const horaSalidaStr = new Date(registro.salida).toLocaleTimeString("es-ES", { hora: "2 dígitos", minutos: "2 dígitos" });
    const hs = parseFloat(registro.hsTrab || 0);
    devolver {estado: hs >= 8? "completoExtra" : "completo", tarde, minutosTarde, horaLlegada: registro.horaReg, horaSalida: horaSalidaStr, horasTrabajadas: hs.toFixed(2), registroRef: registro, fecha: dateStr };
  };

  const statusConfig = {
    en_turno: { label: "En turno", color: "#10b981", bg: "#10b98120" },
    completo: { etiqueta: "Completó", color: "#06b6d4", bg: "#06b6d420" },
    completoExtra: { label: "Completó + Extra", color: "#8b5cf6", bg: "#8b5cf620" },
    horas_extra: { label: "⚡ Horas Extra", color: "#f59e0b", bg: "#f59e0b20" },
    falta: { etiqueta: "Falta", color: "#ef4444", bg: "#ef444420" },
    permiso: {etiqueta: "Permiso", color: "#94a3b8", bg: "#94a3b820" },
    sin_registrar: {etiqueta: "Sin registrador", color: "#475569", bg: "#47556920" },
  };

  const marcarPermiso = () => {
    si (!permisoModal) regresar;
    ...
    guardar(nueva); setPermisoModal(nulo); setPermisoNota(""); flash("✅ Permiso registrado");
  };
  ...
  const resolverAlerta = (tipo) => {
    si (!alertaModal) regresar;
    const { empId, registroRef } = alertaModal;
    dejar horaFin;
    si (tipo === "manual") {
      if (!horaSalidaManual) { flash("Ingresa la hora de salida", "err"); devolver; }
    ...
    const nuevaEmpresa = { ...empresa, empleados: empresa.empleados.map(e => e.id === empId ? { ...e, horasTrabajadas: (e.horasTrabajadas || 0) + hs, registros: e.registros.map(r => r === registroRef ? { ...r, salida: horaFin.toISOString(), hsTrab: hs.toFixed(2), resolución: tipo } : r) } : e) };
    guardar(nuevaEmpresa); setAlertaModal(nulo); setHoraSalidaManual(""); flash(`✅ Resultado como ${tipo === "extra" ? "horas extra" : "salida manual"}`);
  };

  // ── PRÓXIMAS CUOTAS ──
  const getProximasCuotas = (datos, dias = 7) => {
    const hoy = nueva fecha(); hoy.setHours(0,0,0,0);
    límite constante = nueva fecha(); limite.setDate(hoy.getDate() + dias);
    const resultado = [];
  ...
  const exportarAsistencia = (periodo) => {
    const fechas = obtenerFechasParaElPeriodo(periodo);
    const filas = [];
  ...
  const exportarReporteNomina = () => {
    ...
    allEmpresas.forEach(e => {
      (e.datos?.empleados || []).forEach(emp => {
        ...
        resumen.push({
          ...
          "Total Préstamos": ps.duración,
          "Préstamos Activos": activos.longitud,
          "Monto por Cobrar": porCobrar.toFixed(2),
          "Estado": activos.longitud > 0 ? "Con deuda": "Solvente",
        });
      });
    });

    // Hoja 2: Detalle préstamos
    const prestamos = [];
    allEmpresas.forEach(e => {
    ...
    // Ficha 3: Próximas a vencer (7 días)
    const proximas = [];
    allEmpresas.forEach(e => {
      getProximasCuotas(e.datos, 7).forEach(item => {
        proximas.push({
          ...
          "Monto Cuota": elemento.cuota.monto,
  ...
  const calcCreditScore = (empId, datos) => {
    ...
    if (vencidas > 0) return { label: "No recomendado", color: "#ef4444", bg: "#ef444422", icon: "🔴", puntaje: 0, detalle: { total, alDia, tarde, vencidas, pagados } };
    if (tarde > 0) return { label: "Regular", color: "#f59e0b", bg: "#f59e0b22", icon: "🟡", puntaje: 1, detalle: { total, alDia, tarde, vencidas, pagados } };
    return { label: "Excelente", color: "#10b981", bg: "#10b98122", icon: "⭐", puntaje: 2, detalle: { total, alDia, tarde, vencidas, pagados } };
  };
  ...
  // PRINCIPAL
  si (pantalla === "principal") devolver (
    <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p style={{ color: "#10b981", fontSize: "11px" }} className="mt-1">por Nómina Activa</p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setScreen("nomina_login")} className="py-3 px-4 rounded-lg" style={{ backgroundColor: "#1e293b" }} title="Nombre Activa">
  ...
  // ── BLOQUE DE VERIFICACIÓN (en línea, no como subcomponente, para evitar la pérdida de foco) ──
  ...
  // ADELANTO paso 4
  si (pantalla === "adelanto" && loanStep === 4) devolver (
    <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white flex items-center justify-center p-4">
  ...
  // ADELANTO paso 3 (términos)
  if (pantalla === "adelanto" && lendStep === 3 && empActual) {
    const préstamo = calcLoan(loanMonto, loanPlazo, loanFreq);
    devolver (
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        <div className="max-w-md mx-auto mt-6">
          ...
          <div className="rounded-xl p-6" style={{ backgroundColor: "#1e293b", borderTop: "4px solid #f59e0b" }}>
            ...
            <FlashMsg msg={msg}/>
            {[["Monto solicitado", `$${loan.monto}`, null], [`Costo (5% × ${loanPlazo} mes${loanPlazo > 1 ? "es" : ""})`, `$${loan.int}`, "#f59e0b"], ["Total a devolver", `$${loan.total}`, "#ef4444"], [`${préstamo.n} cuota${préstamo.n > 1 ? "s" : ""} ${loanFreq === "mensual" ? "mensuales" : freqpréstamo === "quincenal" ? "quincenales" : "semanales"}`, `$${loan.cuota} c/u`, "#10b981"]].map(([l, v, c]) => (
  ...
  // ADELANTO paso 2
  if (pantalla === "adelanto" && lendStep === 2 && empActual) {
    const limite = getLimite(empleadoId);
    const préstamo = préstamoMonto ? calcLoan(préstamoMonto, préstamoPlazo, préstamoFreq) : null;
    devolver (
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        <div className="max-w-md mx-auto mt-6">
          ...
          <div className="rounded-xl p-6" style={{ backgroundColor: "#1e293b", borderTop: "4px solid #f59e0b" }}>
            ...
            <button onClick={() => {
              if (!loanMonto || parseFloat(loanMonto) <= 0) { flash("Ingresa un monto válido", "err"); devolver; }
              if (parseFloat(loanMonto) > parseFloat(limite)) { flash(`Máximo disponible: $${limite}`, "err"); devolver; }
              establecerPasoDePréstamo(3);
  ...
  // INICIO DE SESIÓN DE JEFE
  si (pantalla === "jefe_login") devolver (
    <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <BackBtn onClick={goHome}/>
  ...
  si (pantalla === "jefe") {
    ...
    // Panel
    si (jefeSub === "dashboard") {
      const fechas = obtenerFechasParaPeriodo(dashPeriodo);
      ...
      empsList.forEach(emp => {
        fechas.forEach(d => {
          ...
          si (st.tarde) totalTarde++;
          if (["completeExtra","horas_extra"].includes(st.status)) totalExtra++;
        ...
        <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
          ...
          <div className="max-w-2xl mx-auto mt-4">
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-xl font-bold" style={{ color: "#06b6d4" }}>Panel de control · {empresa?.nombre}</h2>
                ...
                {horaEntradaEdit && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    }} className="px-3 py-1 rounded text-white text-xs font-bold" style={{ backgroundColor: "#10b981" }}>Aceptar</button>
            ...
            {(() => {
              const próximas = getProximasCuotas(empresa, 7);
              const vencidas = getVencidasSinPagar(empresa);
              Si (!proximas.length && !vencidas.length) devuelve null;
              devolver (
                <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: "#0f1a0f", border: "1px solid #10b98144" }}>
                  <p className="text-sm font-bold mb-2 flex items-center gap-2" style={{ color: "#10b981" }}>
                    <Bell size={14}/>Descuentos en Nomina
                  </p>
                  {vencidas.length > 0 && <p className="text-xs mb-1" style={{ color: "#ef4444" }}>⚠️ {vencidas.length} cuota{vencidas.length > 1 ? "s" : ""} vencida{vencidas.longitud > 1 ? "s" : ""} sin cobrar — aplicar descuento urgente</p>}
                  {proximas.map((item, i) => (
            ...
            <div className="space-y-4">
              {empsList.map(emp => {
                ...
                const horasPeriodo = statuses.reduce((suma, st) => suma + (st.horasTrabajadas ? parseFloat(st.horasTrabajadas) : 0), 0);
                  ...
                  <div key={emp.id} className="rounded-xl p-4" style={{ backgroundColor: "#1e293b" }}>
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <p className="text-xs" estilo={{ color: "#64748b" }}>{s? `$${s.salarioMensual}/mes` : "Sin salario"}{emp.telefono ? ` · ${emp.telefono}` : ""}</p>
                      </div>
                      <div className="flex flex-wrap gap-1 justify-end max-w-40">
                        {ps.length > 0 && <span className="px-2 py-0.5 rounded text-xs bg-yellow-500/20 text-yellow-400">Prestamo</span>}
                    ...
                    {ps.length > 0 && (
                      <div className="mt-3 pt-2" style={{ borderTop: "1px solid #334155" }}>
                        ...
                        {ps.map(p => { const c = p.cuotas.find(c => c.estado === "pendiente"); return c ? <p key={p.id} className="text-xs" style={{ color: "#94a3b8" }}>Cuota {c.num}/{p.nCuotas}: <strong style={{ color: "white" }}>${c.monto}</strong> · vence {c.vence}</p> : nulo })}
                      </div>
                    )}
                  </div>
                );
    ...
    if (jefeSub === "empleados") return (
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        {confirmar && <ConfirmModal text={confirm.text} onYes={confirm.onYes} onNo={() => setConfirm(null)}/>}
        <div className="max-w-md mx-auto mt-4">
          <BackBtn onClick={() => { setJefeSub("menú"); clearEmpForm(); }} etiqueta="Jefe de Panel"/>
          ...
          <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: "#1e293b" }}>
            <h3 className="font-bold mb-3 text-sm" estilo={{ color: editEmpId? "#f59e0b": "#10b981" }}>{editEmpId? "✏️ Editar Empleado" : "➕ Nuevo Empleado"}</h3>
                ...
                <div key={emp.id} className="rounded-lg p-4" style={{ backgroundColor: "#1e293b" }}>
                  <div className="flex justify-between items-start">
                    <div className="flex-1 mr-2">
                      ...
                      <p className="text-xs" style={{ color: "#64748b" }}>{emp.telefono || "Sin tel"} · {emp.cédula || "Sin cédula"}</p>
    ...
    // Cambiar Clave
    si (jefeSub === "clave") devolver (
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        <div className="max-w-md mx-auto mt-6">
          <BackBtn onClick={() => { setJefeSub("menú"); setClaveActual(""); setClaveNueva(""); setClaveConf(""); }} etiqueta="Jefe de Panel"/>
          <div className="rounded-xl p-6" style={{ backgroundColor: "#1e293b" }}>
            ...
            <button onClick={() => {
              if (!claveActual || !claveNueva || !claveConf) { flash("Completa todos los campos", "err"); devolver; }
              if (claveActual !== empresa?.claveJefe) { flash("La clave actual es incorrecta", "err"); devolver; }
              if (claveNueva !== claveConf) { flash("Las claves nuevas no coinciden", "err"); devolver; }
              if (claveNueva.length < 4) { flash("La clave debe tener mínimo 4 caracteres", "err"); devolver; }
              save({ ...empresa, claveJefe: claveNueva });
              setClaveActual(""); setClaveNueva(""); setClaveConf("");
              flash("✅ Clave correctamente actualizada");
      ...
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        <div className="max-w-md mx-auto mt-6">
          ...
          {(() => {
            ...
            devolver (
              <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: "#1a0f00", border: "1px solid #f59e0b44" }}>
                ...
                {vencidas.length > 0 && (
                  <p className="text-xs mb-1" style={{ color: "#ef4444" }}>⚠️ {vencidas.length} cuota{vencidas.length > 1 ? "s" : ""} vencida{vencidas.longitud > 1 ? "s" : ""} sin cobrar</p>
                )}
                {proximas.length > 0 && (
                  <p className="text-xs" style={{ color: "#f59e0b" }}>🔔 {proximas.length} cuota{proximas.length > 1 ? "s" : ""} vence{proximas.longitud === 1 ? "" : "n"} en los próximos 7 días</p>
                )}
          ...
          <div className="rounded-xl p-6" style={{ backgroundColor: "#1e293b" }}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <label className="cursor-pointer" title="Subir logo de empresa">
                ...
                <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                  ...
                  const lector = nuevo lector de archivos();
                  lector.onload = (ev) => guardar({ ...empresa, logo: ev.target.result });
                  lector.readAsDataURL(archivo);
                }}/>
              </label>
            </div>
            <div className="space-y-2">
              {[
                ["Gestionar Empleados", Usuarios, "#10b981", () => setJefeSub("empleados")],
                ["Ver Panel", BarChart3, "#06b6d4", () => setJefeSub("panel")],
  ...
  // INICIO DE SESIÓN DE NOMINA
  si (pantalla === "nomina_login") devolver (
    <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <BackBtn onClick={goHome}/>
        <div className="rounded-xl p-8" style={{ backgroundColor: "#1e293b" }}>
          ...
          <div className="relative mb-4">
            <input type={showPass ? "text" : "password"} placeholder="Clave maestra" value={naPass}
              onChange={e => setNaPass(e.target.value)} onKeyDown={e => e.key === "Entrar" && loginNA()}
              className="w-full px-4 py-3 rounded-lg text-black pr-12" style={{ backgroundColor: "#e2e8f0" }}/>
            <button onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500">{showPass ? <EyeOff size={18}/> : <Eye size={18}/>}</button>
          </div>
  ...
  si (pantalla === "nomina") {
    ...
    // ── Tarjeta NA (debe ser antes del cheque de empleado) ──
        ...
        <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
          <div className="max-w-md mx-auto mt-4">
            <div className="flex justify-between items-center mb-0">
              <BackBtn onClick={() => { setNaView("empresa"); setSelEmpId2(nulo); }} etiqueta={selEmpresa.nombre}/>
            ...
            <div className="rounded-xl p-5 mb-4" style={{ backgroundColor: "#1e293b" }}>
              ...
              <div className="grid grid-cols-1 gap-2 mb-3">
                {[
                  [Teléfono, "Teléfono", empData?.telefono || "No registrado"],
                  [Correo, "Correo", empData?.correo || "No registrado"],
                  ["CI", "Cédula", empData?.cédula || "No registrado"],
                  [MapPin, "Dirección", empData?.direccion || "No registrado"],
            ...
            {/* Préstamos */}
            {/* Tarjeta de puntuación crediticia */}
            {(() => {
                ...
                <div className="space-y-4">
                  {ps.map(p => (
                    <div key={p.id} className="rounded-xl p-4" style={{ backgroundColor: "#1e293b" }}>
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <span className="text-xs px-2 py-1 fuente redondeada-bold" style={{ backgroundColor: p.estado === "pagado" ? "#10b98122" : "#f59e0b22", color: p.estado === "pagado" ? "#10b981": "#f59e0b" }}>
                      ...
                      {/* Teléfono al momento del préstamo */}
      ...
      devolver (
        <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
          <div className="max-w-md mx-auto mt-4">
            ...
            <p className="text-xs mb-4" style={{ color: "#64748b" }}>Desde: {d?.fechaCreacion ? nueva Fecha(d.fechaCreacion).toLocaleDateString("es-ES") : "N/A"}</p>
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ["Empleados", d?.empleados?.longitud || 0, "#10b981"],
    ...
    // Todas las empresas
    const gTotal = todasEmpresas.reduce((s, e) => s + (e.datos?.prestamos?.reduce((a, p) => a + p.montoSol, 0) || 0), 0);
    ...
    // Ayudantes de gráficos
    const pct = (v, t) => t > 0 ? Math.round((v / t) * 100) : 0;
    const activosPct = pct(gActivos, totalPréstamos);
    const pagadosPct = pct(gPagados, totalPréstamos);
    ...
    devolver (
      <div style={{ backgroundColor: "#0f172a", minHeight: "100vh" }} className="text-white p-4">
        <div className="max-w-lg mx-auto mt-4">
            ...
            <div className="rounded-xl p-4 mb-5" style={{ backgroundColor: "#1e293b" }}>
              ...
              <div className="flex items-center gap-4 mb-4">
                <svg width="90" height="90" viewBox="0 0 90 90">
                  ...
                  <círculo cx="45" cy="45" r="35" relleno="ninguno" trazo="#10b981" anchoTrazo="14"
                    trazoDasharray={`${pagadosPct * 2.199} ${219.9 - pagadosPct * 2.199}`}
                    strokeDashoffset="54.975" strokeLinecap="round"/>
                  <círculo cx="45" cy="45" r="35" relleno="ninguno" trazo="#8b5cf6" anchoTrazo="14"
                    trazoDasharray={`${activosPct * 2.199} ${219.9 - activosPct * 2.199}`}
              ...
              <div className="rounded-full overflow-hidden mb-1" style={{ backgroundColor: "#334155", height: 20 }}>
                <div className="h-full flex items-center justify-center text-xs font-bold text-white rounded-full transition-all"
                  estilo={{ ancho: `${cobradoPct}%`, minWidth: cobradoPct > 0? 40: 0, color de fondo: cobradoPct >= 80? "#10b981": cobradoPct >= 50 ? "#f59e0b": "#ef4444" }}>
          ...
          {/* ── Lista de empresas ── */}
          {/* Próximos pagos en todas las empresas */}
          {(() => {
            ...
            Si (!todasProximas.length && !todasVencidas.length) devuelve null;
            devolver (
              <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: "#1a0f00", border: "1px solid #f59e0b44" }}>
                <p className="font-bold text-sm mb-2 flex items-center gap-2" style={{ color: "#f59e0b" }}>
                  <Tamaño de campana={15}/>Alertas Globales
                </p>
                {todasVencidas.length > 0 && (
                  <div className="mb-2">
                    <p className="text-xs font-bold mb-1" style={{ color: "#ef4444" }}>⚠️ {todasVencidas.length} cuota{todasVencidas.length > 1 ? "s" : ""} vencida{todasVencidas.longitud > 1 ? "s" : ""} sin cobrar</p>
